...
include(joinpath(@__DIR__, "..", "chapter14_orbit_correction_tools.jl"))
include(joinpath(@__DIR__, "..", "14_1_before_optimization", "chapter14_sawtooth_ring_before.jl"))
include(joinpath(@__DIR__, "..", "14_2_after_optimization", "chapter14_sawtooth_ring_after.jl"))
...
horizontal_initial_ip = [
    chapter14_linear_interpolate(bpm_s, x_corrected, ip_s),
    chapter14_local_slope(bpm_s, x_corrected, ip_s),
]

# Representative measured vertical IP error for this exercise.
vertical_initial_ip = [-0.42e-3, 5.5e-6]

horizontal_local_bump = chapter14_local_bump_solution(
    horizontal_bump_s,
    ip_s,
    bump_close_s,
    horizontal_initial_ip;
...
vertical_local_bump = chapter14_local_bump_solution(
    vertical_bump_s,
    ip_s,
    bump_close_s,
    vertical_initial_ip;
...
println("Chapter 14.4 local IP bump")
...
local_bump_solution_path = joinpath(@__DIR__, "chapter14_local_bump_solution.jl")
chapter14_write_local_bump_solution(local_bump_solution_path, local_bump_solution)
