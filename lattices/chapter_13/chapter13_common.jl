...
function chapter13_quad_slices()
    return [Quadrupole(name=@sprintf("Q_%02d", i), L=0.6 / CH13_N_QUAD, Kn1=0.23) for i in 1:CH13_N_QUAD]
end

function build_chapter13_lattice(; b_vkick=0.0, q_hkick=0.0, q_vkick=0.0)
    b_kick = thin_kick("B_KICK"; vkick=b_vkick)
    q_kick = thin_kick("Q_KICK"; hkick=q_hkick, vkick=q_vkick)

    return Beamline(
        vcat(chapter13_drift_slices(), [b_kick], chapter13_bend_slices(), [q_kick], chapter13_quad_slices());
...
function chapter13_state()
    design = build_chapter13_lattice()
    return LatticeState(design, deepcopy(design), deepcopy(design))
end

function chapter13_figure27_state()
    state = chapter13_state()
